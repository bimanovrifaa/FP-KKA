...
def draw_board():
    ...
    hint_val = FONT_STATS.render(str(hint_penalty_count), True, (241, 196, 15)) 
    ...
    clear_w = (btn_w_small * 2) + btn_gap 
    draw_interactive_button("Clear Board", SIDEBAR_X, btn_row2_y, clear_w, btn_h_small, C_BOARD_BG, C_CELL_HOVER, "Clear", mouse_pos, text_color=C_TEXT_ERROR)
...
def click_button_check_game(pos):
    ...
    # Row 2: Clear
    clear_w = (btn_w_small * 2) + btn_gap
    if SIDEBAR_X <= mx <= SIDEBAR_X + clear_w and btn_row2_y <= my <= btn_row2_y + btn_h_small: return "Clear"
...
# --- MODIFIED: ADDED VICTORY SCREEN OVERLAY ---
    ...
    try:
        while running:
            ...
            if game_state == "MENU":
                ...
                draw_board()
                # Overlay Finished untuk Auto-Solver saja (karena manual sudah ditangani check_auto_restart)
